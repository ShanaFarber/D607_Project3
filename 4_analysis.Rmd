...
Based on the job postings that had college degree information, having a Master's was first at 496, followed by a Bachelor's at 456. 
...
```{r top10-ds}
top_10 <- job_listings_skills_long %>%
  count(skill) %>%
  arrange(desc(n)) %>%
  head(n = 10)
...
data_scientist <- job_listings_skills_long %>% 
  filter(str_detect(job_title, "data scientist")) %>%
  mutate(job = "Data Scientist")
  
data_analyst <- job_listings_skills_long %>% 
...
common_skills <- as.vector(data_analyst_top10 |> 
  inner_join(data_scientist_top10, by = 'skill') |> 
  select(skill))

data_scientist_top10 <-
  data_scientist_top10 |> mutate(f_color = ifelse(skill %in% c('C++', 'Python', 'Data visualization', 'Statistics'), "Yes", "No"))

data_analyst_top10 <-
  data_analyst_top10 |> mutate(f_color = ifelse(skill %in% c('C++', 'Python', 'Data visualization', 'Statistics'), "Yes", "No"))

# plot top 10
data_scientist_top10 %>%
  ggplot(aes(y = reorder(skill, n), x = n, fill = f_color)) +
    geom_bar(stat = "identity") +
    labs(title = "Top 10 Skills for Data Scienctists", x = "Count", y='') +
    theme_bw() +
    theme(legend.position = "none") +
...
Sort the data by the number of citations in descending order and select the top 50 most cited skills. Create the plot using ggplot2 and ggwordcloud packages
...
```{r}
exp5_skill_counts <- import_df %>%
  filter(years_exp >= 5) %>%
  group_by(skill) %>%
  summarize(counts = n(), .groups = 'keep')

ggplot(exp5_skill_counts, aes(label = skill, size = counts, color = factor(sample.int(8, nrow(exp5_skill_counts), replace = TRUE)))) +
  geom_text_wordcloud() +
  scale_size_area(max_size = 10) +
  theme_minimal()
```
...
skills_freq <- import_df %>%
  count(skill) %>%
  arrange(desc(n)) %>%
  head(100)

ggplot(skills_freq, aes(label = skill, size = n, color = factor(sample.int(8, nrow(skills_freq), replace = TRUE)))) +
  geom_text_wordcloud() +
  theme_minimal() +
  scale_size_area(max_size = 10) +
...
loc_edu <-
  import_df |> 
  na.omit(continent, highest_ed) |>
  group_by(continent, highest_ed) |> 
  summarise(count = n_distinct(job_id), .groups = "keep")

 loc_edu |> 
  ggplot(aes(y = reorder(continent, count), x = count, fill = continent)) +
  geom_bar(stat = 'identity') +
  labs(title = "Number of job listings by location and highest education level",
       x = "", y = "") +
  theme_bw() +
  theme(legend.position = "none") +
   facet_grid(cols = vars(highest_ed)) +
   scale_fill_brewer(palette = "Dark2") 
   
 knitr::kable(loc_edu |> pivot_wider(names_from = highest_ed, values_from = count))
```
