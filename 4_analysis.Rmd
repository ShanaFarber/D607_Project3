...
skill_counts <- import_df %>%
  ...
  filter(!skills %in% c('engineering', 'data science', 'research', 'programming')) |> 
  arrange(desc(count))

skill_counts |> 
  head(20) |> 
  ggplot(aes(x = count, y = reorder(skills, count))) +
  geom_bar(stat = 'identity', fill = '#4E79A7') +
...
ggplot(skill_counts, aes(label = skills, size = count, color = factor(sample.int(8, nrow(skill_counts), replace = TRUE)))) +
  geom_text_wordcloud() +
