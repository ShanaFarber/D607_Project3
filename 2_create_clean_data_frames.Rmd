...
extractSkills <- function(job_df, dict) {
  ...
  vis_and_r_check <- c("R"="(\\s|\\\\)R(\\s|,|\\\\)", "Data visualization"="visual", "Data collection" = "(Data collection|collect data)", "Data analysis" = "(Data analysis|analyze data)", "Communication" = "(Communication|communicate)", "Collaboration" = "(Collaboration|collaborate)", "C" = "(\\s|\\\\)C(\\s|,|\\\\)", "C\\+\\+" = "C\\+\\+")
  ...
  for(i in 1:length(job_df$job_id)) {
    ...
    for (j in 1:length(dict)) {
      ...
      if (!pat %in% names(vis_and_r_check)) {
        if (str_detect(tolower(job_df$job_quals[i]), tolower(pat))) {
        job_id_and_skill <- append(job_id_and_skill, paste(job_id, pat, sep = ","))
        }
      } else {
        new_pat <- vis_and_r_check[pat]
      
        if (str_detect(tolower(job_df$job_quals[i]), tolower(new_pat))) {
        job_id_and_skill <- append(job_id_and_skill, paste(job_id, pat, sep = ","))
        }
      }
...
write.csv(job_listings, "job_recommendations/job_listings.csv", row.names=F)
